...
class Model:
    ...
    def _create_dual_objective(self, primal, dual):
        f = [x.bound for x in primal.constraints]
        obj_exp = Expression.from_vectors(dual.variables, f)
        dual.minimize(obj_exp)


    def _create_dual_constraints(self, primal, dual):
        pr_factors = np.array([[atom.factor for atom in constraint.expression.atoms] for constraint in primal.constraints])
        dual_factors = pr_factors.T
        for i, row in enumerate(dual_factors):             
            con_exp = Expression.from_vectors(dual.variables, row)
            constraint = cons_exp >= primal.objective.expression.atoms[i].factor
