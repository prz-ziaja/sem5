...
class Constraint:
    ...
    def __init__(self, expression, bound, type = ConstraintType.GE):
        if bound < 0 and type == ConstraintType.LE:
            raise Exception("The SAPORT linear solver doesn't support negative bounds of type LE")
        if bound > 0 and type == ConstraintType.GE:
            raise Exception("The SAPORT linear solver doesn't support positive bounds of type GE")
